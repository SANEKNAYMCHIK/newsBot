...
cp .env.example .env
# Отредактировать .env, добавив TOKEN для телеграм и ключ для JWT токена

# 3. Создать файл конфигурации для фронтенда
cd frontend
cp .env.example .env

# 4. Запустить все сервисы со сборкой (бэкенд + БД)
cd ..
docker-compose up -d --build

# 5. Запустить фронтенд (в новом терминале)
cd frontend
npm install
npm start
```
